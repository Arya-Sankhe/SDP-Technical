# %% [markdown] cell 0
# Experiment: MSFT 1-Minute GRU Forecast (Ensemble Trend Injection v7.5)

Key changes for v7.5 - Smarter Inference with Trend Matching:
1. Generate 20 different paths using generate_realistic() with different random seeds
2. Calculate trend slope for each path over the 15-step horizon
3. Calculate trend slope of the last 20 bars of input context (historical trend)
4. Select the generated path whose trend slope is closest to historical trend (momentum continuation)
5. If historical trend is strong (slope > 0.002), reject paths that go opposite direction
6. Add 'momentum confidence' visualization showing which path was selected and why
